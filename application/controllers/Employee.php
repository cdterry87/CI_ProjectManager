...
class Employee extends CI_Controller
{
    ...
    public function login()
    {
        ...
        //Verify if user is already logged in, and if so, redirect.
        if ($this->logged_in()) {
            $this->redirection();
        }
    ...
    /**
     * If a URI was attempted before logging in, after successful authentication, try to redirect to the attempted URI.
     */
    public function redirection()
    {
        $current_uri=$this->session->userdata('current_uri');
        if (trim($current_uri)!='') {
            //Redirect to the URI that was attempted.
            $this->session->unset_userdata('current_uri');
    ...
    public function set_sessions($user)
    {
        ...
        if (!empty($user)) {
            ...
            $session=array(
                ...
                'employee_sales'        => $user['employee_sales'],
